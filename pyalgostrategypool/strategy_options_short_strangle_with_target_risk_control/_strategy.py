"""
   Strategy Description:
       The Options Short Strangle strategy involves selling an Out-of-the-Money (OTM) Call and an OTM Put option with the same expiration but different strike prices.
       It aims to profit when the underlying asset stays within a defined price range, making it suitable for low to moderate volatility conditions.
       The strategy benefits from time decay and earns a net premium, with maximum profit realized if the underlying expires between the two strike prices.
       However, it carries risk beyond the breakeven points if the price moves sharply in either direction.
"""
...
class StrategyOptionsShortStrangleWithTargetRiskControl(StrategyOptionsBase):
    ...
    def validate_parameters(self):
        ...
        check_argument(self.number_of_strikes, "extern_function", is_positive_int, "Value should be positive integer")  # Validate number of strikes
