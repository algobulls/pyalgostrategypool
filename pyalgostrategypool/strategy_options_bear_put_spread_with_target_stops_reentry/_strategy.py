...
Strategy Description:
   The Options Bear Put Spread strategy establishes a bearish position by buying an at-the-money (ATM) Put option and selling a lower-strike Put option with the same expiry.
   This variant adds a target profit, a hard stop-loss, and a trailing stop-loss to manage downside exposure and capture extended bearish moves.
   It also allows controlled re-entries when the exit conditions are met and market conditions justify a new entry.
"""


class StrategyOptionsBearPutSpreadWithTargetStopsAndReentry(StrategyOptionsBase):
    name = "Strategy Options Bear Put Spread With Target Stops And Re-entry"

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        # Required parameters for this strategy
        self.no_of_otm_strikes_leg_sell = self.strategy_parameters.get("NUMBER_OF_OTM_STRIKES_SELL_LEG", 2)
        self.re_entry_limit = self.strategy_parameters.get('RE_ENTRY_LIMIT', 2)
        self.stoploss_percentage = self.strategy_parameters.get('STOPLOSS_PERCENTAGE', 25)
        self.target_percentage = self.strategy_parameters.get('TARGET_PERCENTAGE', 10)
        self.tsl_percentage = self.strategy_parameters.get("TRAILING_STOPLOSS_PERCENTAGE", 3)
        ...
        self.child_instrument_main_orders = None  # Tracks Put orders
        self.highest = None
        self.reentry_count = None
    ...
    def _check_exit_conditions(self, base_instrument, child_leg_orders_dict):
        """
        Evaluate all exit rules for the Bear Put Spread.
        ...
        # Bear Put Spread value = Long(ATM PE) − Short(OTM PE)
    ...
    def _exit_all_positions_for_base_instrument(self, base_instrument):
        ...
        self.child_instrument_main_orders.pop(base_instrument, None)  # Remove references to the base instrument after exiting PE orders.
        self.reentry_count[base_instrument]['reentry_recorded'] = False  # Reset re-entry flag so new entries are allowed after full exit.
        ...
        for instrument in instruments_bucket:
            if not self.reentry_complete.setdefault(instrument, False):
                self.logger.info(
                    ...
                    f"Determining ATM/OTM option instruments and verifying if PE orders are already placed."
                )
                ...
                self.reentry_count.setdefault(instrument, {'reentry_recorded': False})

                # Retrieve total entries so far; skip if re-entry limit reached
                re_entry_count = self.reentry_count[instrument].get('reentry_count', 0)
                ...
                for action, strike_direction, no_of_strikes in leg_wise_list:
                    ...
                    self.options_instruments_set_up(instrument, OptionsInstrumentDirection.EXACT, expiry_date, OptionsTradingsymbolSuffix.PE.value, base_instrument_ltp)

                    # Retrieve ATM/OTM child instrument for the given instrument
                    child_instrument = self.get_options_instrument_with_strike_direction(instrument, expiry_date, OptionsTradingsymbolSuffix.PE.value, strike_direction, no_of_strikes)
    ...
    def strategy_enter_position(self, candle, instrument, meta):
        ...
        if check_order_placed_successfully(_order):
            ...
            if not self.reentry_count[base_instrument].get('reentry_recorded'):
                self.reentry_count[base_instrument]['reentry_count'] = self.reentry_count[base_instrument].get('reentry_count', 0) + 1
                self.reentry_count[base_instrument]['reentry_recorded'] = True
        else:
    ...
    def strategy_select_instruments_for_exit(self, candle, instruments_bucket):
        ...
        for instrument in instruments_bucket:
            if self.instruments_mapper.is_child_instrument(instrument):
                ...
                # Check if PE orders are complete and if trailing stop-loss condition is met.
