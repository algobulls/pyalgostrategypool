"""
    Strategy Description:
        The Options Strangle Intraday strategy involves buying both a call and a put option with different strike prices, but the same expiration, expecting significant price movement in either direction.
        It benefits from large price fluctuations in the underlying asset, regardless of the direction.
        This strategy is suitable for volatile markets where the magnitude of movement is expected to exceed the combined premium of the options.
...
class StrategyOptionsStrangleIntraday(StrategyOptionsBase):
    name = 'Strategy Options Strangle Intraday'

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        # Parameters (currently set with default values, can be overridden from the Parameters Configuration Pane)
        self._leg_one_transaction_type = self.strategy_parameters.get('LEG_ONE_TRANSACTION_TYPE', 2)  # BUY: 1 | SELL: 2
        self._leg_one_tradingsymbol_suffix = self.strategy_parameters.get('LEG_ONE_TRADING_SYMBOL_SUFFIX', 1)  # CE: 1 | PE: 2
        self._leg_one_strike_direction = OptionsStrikeDirectionMap[self.strategy_parameters.get('LEG_ONE_STRIKE_DIRECTION', 2)]  # ITM: 0| ATM: 1| OTM: 2
        self._leg_one_number_of_strikes = self.strategy_parameters.get('LEG_ONE_NUMBER_OF_STRIKES', 2)

        self._leg_two_transaction_type = self.strategy_parameters.get('LEG_TWO_TRANSACTION_TYPE', 2)  # BUY: 1 | SELL: 2
    ...
    def strategy_enter_position(self, candle, instrument, meta):
        _ = self.broker.OrderRegular(instrument, meta['action'], quantity=self.number_of_lots * instrument.lot_size)
        return _
